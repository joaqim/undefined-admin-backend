...
const { FORTNOX_CLIENT_ID, FORTNOX_CLIENT_SECRET, FORTNOX_REDIRECT_URI } = process.env;
  ...
  try {
    ...
    if (grant_type === 'authorization_code') {
      ...
      if (!redirect_uri && !FORTNOX_REDIRECT_URI) {
        return res.status(400).send({ error: 'Param `redirect_uri` is missing for Authorization' });
      }
      ...
      return res.status(400).send({ error: "Param `grant_type` expected to be 'authorization_code'/'refresh_token' " });
    }
